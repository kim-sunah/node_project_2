...
authRouter.post('/signup', async (req, res) => {
    try {
        ...
        const isUsesr = await users.findOne({ where: { email } })
        ...
        const newUser = await users.create({ email, password: hashPassword, name })
...
authRouter.post('/signin', async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id }, JWT_ACCESS_TOKEN_SECRET, { expiresIn: JWT_ACCESS_TOKEN_EXPIRES_IN });
